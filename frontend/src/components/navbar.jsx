...
const Navbar = () => {
  ...
  useEffect(() => {
    const verifyToken = async (token) => {
      try {
        const response = await fetch("https://anigalaxy-final-1.onrender.com/api/v1/user/verify-token", {
          method: "GET",
          headers: {
        ...
        return response.ok && data.validToken; // Return true if valid
      } catch (error) {
            ...
            <ul className="flex space-x-8 text-lg text-slate-300 gap-8 items-center">
              ...
              <li>
                {isLoggedin ? (
                  ...
                  <Link to="/signin" className={`block py-2 ${isActive("/signin")}`}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
